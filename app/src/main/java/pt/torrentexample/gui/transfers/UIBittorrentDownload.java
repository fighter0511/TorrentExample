...
import frostwire.bittorrent.BTDownload;
import frostwire.bittorrent.BTDownloadItem;
import frostwire.bittorrent.BTDownloadListener;
...
import pt.torrentexample.core.provide.TableFetcher;
import pt.torrentexample.core.provide.TableFetchers;
...
public final class UIBittorrentDownload implements BittorrentDownload {
    ...
    private long calculateSize(BTDownload dl) {
        ...
        if (partial) {
            List<frostwire.transfers.TransferItem> items = dl.getItems();

            long totalSize = 0;
            for (frostwire.transfers.TransferItem item : items) {
                if (!item.isSkipped()) {
