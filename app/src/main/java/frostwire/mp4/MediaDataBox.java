 ...
 * by the metadata (see {@link com}).<br>
 ...
 * metadata refers to media data by its absolute offset within the file (see});
...
public final class MediaDataBox implements Box {
    ...
    private static void transfer(DataSource from, long position, long count, WritableByteChannel to) throws IOException {
        ...
        long offset = 0;
        while (offset < count) {
            offset += from.transferTo(position + offset, java.lang.Math.min(maxCount, count - offset), to);
        }
    }
